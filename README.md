...
| 2.4 Signal Quality | MCD / PESQ / STOI against a **matched-speaker reference**. Enabled by default — `setup.sh` clones a single-speaker corpus (LJSpeech) into a Kugel voice via `scripts/clone_reference_voice.py` and wires it into the config; measures **clone fidelity** (how faithfully Kugel reproduces the cloned speaker). Self-skips only if the clone step was skipped (no API key/HF at setup). |
